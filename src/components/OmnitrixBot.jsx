...
function matchQuery(text) {
    ...
    const keywords = {
        rules: ["rule", "regulation", "operational", "guideline", "allowed", "prohibited"],
        coordinates: ["venue", "time", "when", "where", "location", "place", "lab"],
        squad: ["team", "squad", "member", "size", "laptop", "requirement"],
        command: ["coordinator", "contact", "phone", "number", "channel", "call"],
    };
    for (const [id, words] of Object.entries(keywords)) {
        if (words.some((w) => lower.includes(w))) return chatQueries.find((q) => q.id === id);
    }
    return null;
}
...
export default function OmnitrixBot() {
    ...
    const handleQuery = (query, userLabel) => {
        ...
        const userMsg = { id: `user-${Date.now()}`, sender: "user", content: userLabel || query.label, timestamp: now };
        const response = query.getResponse();
        const botMsg = { id: `bot-${Date.now()}`, sender: "bot", content: response.title + "\n\n" + response.items.join("\n"), timestamp: now };
    ...
    const handleSubmit = (e) => {
        ...
        if (match) {
            ...
            const now = formatTime();
            setMessages((prev) => [...prev, { id: `user-${Date.now()}`, sender: "user", content: text, timestamp: now }]);
            setTimeout(() => {
                ...
                {isOpen && (
                    <motion.div
                        style={{
                            ...
                            height: "min(530px, calc(100vh - 140px))",
                    ...
                    >
                        ...
                        <div style={{
                            ...
                            {chatQueries.map((q) => (
                                <motion.button
                                    ...
                                    whileHover={{ scale: 1.04, borderColor: "rgba(57,255,20,0.4)", color: "#39FF14" }}
                                    whileTap={{ scale: 0.96 }}
                                    style={{
                                        padding: "8px 12px",
                                        borderRadius: 9999,
                        ...
                        >
                            ...
                            <motion.button
                                type="submit"
                                whileHover={{ scale: 1.08, boxShadow: "0 0 15px rgba(57,255,20,0.3)" }}
                                whileTap={{ scale: 0.92 }}
                                style={{
                                    width: 42, height: 42, borderRadius: "50%",
                                    border: "1px solid rgba(57,255,20,0.25)",
                                    boxShadow: inputText.trim() ? "0 0 12px rgba(57,255,20,0.15)" : "none",
